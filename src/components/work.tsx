...
export default function Work()
{
    ...
    const Details = [
        {
            ...
            content:["Daily Caffine is a Web Application, developed using Next.js, Next Auth, MongoDb, Tailwind, MUI.",
        ...
        },
        {
            ...
            content:["Course Selling App is a Web Application, developed using MERN Stack, Recoil, MUI.",
    ...
    return(<div style={{cursor:"none"}} className="p-12 ">
        <div>
            <h1 style={{fontFamily:"Saira Condensed"}} className="text-[60px] my-4">My Work</h1>
        ...
        <div key={index} className="flex justify-end m-8 my-16">
          ...
          <div ref={ref} className="h-full">
            ...
            <div className="w-2/4 mx-20">
                <Project Details={Details[index]}></Project>
            </div>
        </div>
      ))}
        </div>
    ...
    <div className="w-full relative h-[350px] z-[100000] RightToLeft ">
        ...
        <img onMouseEnter={()=>{
            setSrc(Details.image2);
                ...
                }} style={{height:"290px"}} className="duration-300 origin-top-left hover:-rotate-3 w-full h-full rounded-l" src={src} alt="" />
        </div>
        
    </div>
    <div className="p-8">
        <div>{Details.content.map((line)=>{
            return (<div style={{fontFamily:"monospace"}} className="my-2 text-lg">{line}</div>)
        })}</div>
    </div>
    </>)
}